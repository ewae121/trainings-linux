...
get_pokemon (){
  ...
  properties_file="${POKEMON_FOLDER}"/"${pokemon_name}${POKEMON_PROPERTIES_FILE_SUFFIX}"
...
get_pokemon_list (){
  ...
  if [[ -f $POKEMON_LIST_FILE ]]; then
    send_activity_message "The pokemon list is already available in $POKEMON_LIST_FILE"
    return
  fi
  curl -s https://pokeapi.co/api/v2/pokemon?limit=12000 | jq > $POKEMON_LIST_FILE
  send_activity_message "the pokemon list is available in $POKEMON_LIST_FILE"
}

download_pokemon_list (){
  cat "${POKEMON_LIST_FILE}" | jq -r '.results[] | join(" ")' | while read -r pokemon url; do
